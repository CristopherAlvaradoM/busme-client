...
export default function AdminLayout({
  ...
  return (
    <div className="font-poppins bg-complementary-100 w-full min-h-screen">
      {/* Botón para abrir el sidebar en pantallas pequeñas */}
      <div className="flex w-full justify-end px-5 py-5 bg-complementary-100 md:hidden">
      ...
      {isSidebarOpen && (
        <div
          className="fixed top-0 left-0 z-40 w-full h-full bg-muted-950 bg-opacity-60"
          onClick={toggleSidebar} // Cerrar el sidebar al hacer clic en la capa de color gris
        />
      )}
      {/* Contenido */}
      <div className={`ml-0 md:ml-64 px-5 md:py-5 max-w-full h-full ${isSidebarOpen && 'overflow-hidden'}`}>
        {children}
      </div>
    </div>
  );
